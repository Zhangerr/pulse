...
namespace Pulse.UI
{
    ...
    public class PTextBox : Control
    {
        ...
        int scrollbackpos = -1;
        const int spacing = 20; //pixel spacing between lines
        ...
        {
            bg.move(new Point(bg.Bounds.X, bg.Bounds.Y + bg.Bounds.Height), time/3);
            bg.fade(0, time);
        ...
        public override void OnRenderFrame(OpenTK.FrameEventArgs e)
        {
            ...
            {
                bg.draw(e); //required so if it starts transitioning again, will update (really should be moved to an update() method tbh)
            }
        }
        public override void OnUpdateFrame(OpenTK.FrameEventArgs e)
        {
            ...
            foreach (Tab t in toRemove)
            {
                ...
                if (t.tabRect.Colour.A <= 0 || !t.tabRect.Fading) //t.tabRect.Alpha doesn't update when fading, need to 
                {
                    tabs.Remove(t.Title);
        ...
        public void parse()
        {
            try
            {
                ...
                while (matchResults.Success)
                {
                    ...
                    r.Colour = Color.FromArgb(100, Color.SteelBlue);
